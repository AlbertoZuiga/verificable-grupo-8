...
def test_edit_course_success(client):
    course = Course.query.first()
    data = {"title": "Matemáticas Avanzadas", "code": "MAT201", "credits": 5}
    with client:
...
def test_edit_keep_code_self(client):
    ...
    response = client.post(
        url_for("course.edit", course_id=course.id), data=data, follow_redirects=True
    )
...
def test_edit_duplicate_code_other_course(client):
    ...
    with client:
        response = client.post(url_for("course.edit", course_id=target_course.id), data=data)
        messages = get_flashed_messages()
