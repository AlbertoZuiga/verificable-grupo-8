...
def index(section_id):
    ...
    user_sections = StudentSection.query.filter_by(section_id=section_id).all()
...
def add_user(section_id):
    ...
    if request.method == 'POST':
        ...
        if not user_id or not role:
            ...
            new_user_section = StudentSection(user_id=user_id, section_id=section_id, role=role)
            try:
...
def remove_user(section_id, user_id):
    user_section = StudentSection.query.filter_by(section_id=section_id, user_id=user_id).first_or_404()
    try:
