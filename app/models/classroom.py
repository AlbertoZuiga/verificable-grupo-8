...
class Classroom(kanvas_db.Model):  # type: ignore[name-defined]
    __tablename__ = "classrooms"

    id = kanvas_db.Column(kanvas_db.Integer, primary_key=True)
    name = kanvas_db.Column(kanvas_db.String(60), nullable=False, unique=True)
    capacity = kanvas_db.Column(kanvas_db.Integer, nullable=False)

    assigned_time_blocks = kanvas_db.relationship(
        "AssignedTimeBlock", back_populates="classroom", cascade="all, delete-orphan"
    )
