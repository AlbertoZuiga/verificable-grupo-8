...
class Student(kanvas_db.Model):
    ...
    # Relationship to sections through association table
    sections = kanvas_db.relationship(
        'Section',
        secondary='student_sections',
        back_populates='students'  # Points to Section.students
    )
    
    # Relationship to StudentSection (for direct access)
    section_associations = kanvas_db.relationship(
        'StudentSection',
