...
class StudentEvaluationInstance(kanvas_db.Model):  # type: ignore[name-defined]
    ...
    evaluation_instance_id = kanvas_db.Column(
        kanvas_db.Integer,
        kanvas_db.ForeignKey("evaluation_instances.id", ondelete="CASCADE"),
        primary_key=True,
    )
    evaluation_instance = kanvas_db.relationship(
        "EvaluationInstance", back_populates="student_evaluation_instances"
    )
